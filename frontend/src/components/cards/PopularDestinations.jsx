...
const PopularDestinations = ({ destination }) => {
    ...
    return (
        <>
            <div
                key={destination.id}
                className={`rounded-lg shadow-md overflow-hidden cursor-pointer transition duration-300 hover:shadow-xl flex flex-col h-full ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}
                data-aos="zoom-in-up"
            >
                <img
                ...
                <div className="p-4 flex flex-col flex-grow">
                    <h3 className={`text-xl font-bold mb-2 ${isDarkMode ? 'text-blue-800' : 'text-gray-700'}`}>
                        {destination.title}
                    </h3>
                    <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-700'} flex-grow`}>
                        {destination.description}
                    </p>
                    <div className="flex justify-center mt-4">
                        <button
                            onClick={() => navigate(`/details/${destination.id}`)}
                            className={`px-5 py-2 text-sm font-semibold border-2 ${isDarkMode ? 'border-gray-600 text-blue-600 hover:border-transparent hover:bg-gray-600' : 'border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white hover:border-transparent'} rounded-full transition duration-300 ease-in-out shadow-md hover:shadow-xl hover:scale-105`}
                        >
                            More Details
                        </button>
                    </div>
