...
const Gallery = () => {
  ...
  return (
    <>
      <div className={`absolute inset-0 bg-gradient-to-b ${isDark ? 'from-gray-900/70 to-black/90' : 'from-white/80 to-gray-200/90'} relative min-h-screen overflow-hidden flex flex-col items-center justify-center text-center px-4 transition-colors duration-500`}>
        <div className="relative h-screen w-full animate__animated animate__fadeIn animate__delay-2s">
          ...
          <div className={`absolute inset-0 bg-opacity-50 flex flex-col items-center justify-center text-center px-4 transition-colors`}>
            <h1 className={`text-4xl md:text-6xl font-extrabold mb-4 tracking-wide drop-shadow-lg flex items-center gap-3 text-${isDark ? "white" : "black"} hover:text-yellow-300 transition-colors duration-500`}>
            ...
            <div className={`flex space-x-6 mt-4 text-3xl text-${isDark ? "white" : "black"} transition-colors duration-500`}>
              {socialLinks.map(({ href, icon, color, label }, index) => {
                ...
                return (
                  <a key={index} href={href} target="_blank" rel="noopener noreferrer" className={`transform transition-transform duration-300 hover:scale-150 ${color}`} aria-label={`Visit our ${label} page transition-colors duration-500`}>
                    <IconComponent />
                  </a>
                );
              })}
        ...
        <div className="container mx-auto px-4 py-10">
          <h2 className={`text-3xl md:text-4xl font-bold text-center text-${isDark ? "white" : "black"} mb-6 transition-colors duration-500`}>Gallery Collection</h2>
          <div className="grid grid-cols-1 md:grid-cols-3  lg:grid-cols-4 gap-4">
            {loading ? pageData.map((img, index) => (
              ...
              <div className="flex items-center justify-center w-full col-span-full min-h-[200px]">
                <span className={`loading loading-dots loading-xl ${isDark ? '' : 'text-black'} transition-colors duration-500`}></span>
              </div>
            )}
          </div>
        </div>
